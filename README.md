...
Для сравнения на Kaggle были отправлены предсказания Dummy baseline, XGBoost baseline и финального stacking.

| Submission | Public Score |
|---|---:|
| Dummy baseline | 2.137 |
| XGBoost baseline | 1.452 |
| Final stacking | **1.251** |

Переход от Dummy baseline к XGBoost снизил public RMSLE на **32.1%**.

Финальный stacking дополнительно улучшил результат XGBoost на **13.8%** и показал лучший public score — **1.251**.

### Основные выводы

1. Временные закономерности и идентификатор здания оказались важнее слабых линейных корреляций погодных признаков с целевой переменной.
2. Циклические признаки `hour_sin` и `hour_cos` устойчиво улучшили все проверенные семейства моделей.
3. Лаговые признаки температуры не дали стабильного улучшения и поэтому не вошли в финальный набор признаков.
4. XGBoost показал лучшее качество среди baseline-моделей.
5. LightGBM обеспечивает наиболее выгодный баланс между качеством и скоростью обучения.
6. MLP с категориальными embeddings показывает конкурентоспособное качество и добавляет ансамблю разнообразие ошибок.
7. Stacking показал лучший результат как на временной validation-выборке, так и на Kaggle.